...
public class ICClientSearchRepository {
	...
	public List<ICClientSearchResultsDto> searchParticipant(
		ICClientSearchCriteriaDto initialContactClientSearchCriteriaDto
	) {
		List<Object> argsObjectList = new ArrayList<>();
		StringBuffer querySBuff = createSearchQuery(
			initialContactClientSearchCriteriaDto,
			argsObjectList
		);
	...
	private StringBuffer createSearchQuery(
		ICClientSearchCriteriaDto searchCriteria,
		...
		querySBuff.append(
			"from participant p left join participantcontact p2 on p.participantid = p2.participantid where  p.status='ACTIVE'"
		);
		if (data != null && !data.trim().isEmpty()) {
			data =
				data
					...
					.replace("[", "![");
			querySBuff.append(
				" AND (p.referenceid =? OR p.dateofbirth LIKE ? OR p.firstname LIKE ? OR p.surname LIKE ?  OR p.middlename LIKE ? OR p.maritalstatus LIKE ? OR p2.homephone LIKE ? OR p2.cellphone LIKE ? OR p2.workphone LIKE ? OR p2.city LIKE ?) "
			);
