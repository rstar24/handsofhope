...
public class ICSearchRepository {
	...
	public List<ICSearchResultsDto> searchInitialContacts(
		ICSearchCriteriaDto initialContactSearchCriteriaDto
	) {
		List<Object> argsObjectList = new ArrayList<>();
		StringBuffer querySBuff = createSearchQuery(
			initialContactSearchCriteriaDto,
			argsObjectList
		);
		return jdbcTemplate.query(
			...
			(rs, rowNum) ->
				new ICSearchResultsDto(
					...
					rs.getDate("startingDate") != null
						? rs.getDate("startingDate").toLocalDate()
						: LocalDate.of(1, 1, 1),
					rs.getString("status")
				)
		);
	...
	private StringBuffer createSearchQuery(
		ICSearchCriteriaDto searchCriteria,
		List<Object> argsObjectList
	) {
		StringBuffer querySBuff = new StringBuffer();
		querySBuff.append(
			"select p.filedetailsid, CONCAT(firstname,' ', surname) AS fullName, p.fileNumber,p.caseworker,p.startingDate,p.status "
		);
		querySBuff.append(
			"from icfiledetails p left join participant p2 on p.clientname = p2.participantid where p.statusofdeletion='ACTIVE'"
		);
