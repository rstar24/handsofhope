...
public class ICContactNotesSearchRepository {
	...
	public List<ICContactNotesSearchResultsDto> searchICContactNotes(
		ICContactNotesSearchCriteriaDto iCContactNotesSearchCriteriaDto
	) {
		List<Object> argsObjectList = new ArrayList<>();
		StringBuffer querySBuff = createSearchQuery(
			iCContactNotesSearchCriteriaDto,
			argsObjectList
		);
	...
	private StringBuffer createSearchQuery(
		ICContactNotesSearchCriteriaDto searchCriteria,
		...
		querySBuff.append(
			"select p.contactnotesid ,p.filedetailsid, p.name ,p.worker ,p.date , p.time,p.contactmethod ,p.needaddress ,p.summary ,p.result ,p.nextstep ,p.caseplanprogress ,p.additionalinformation "
		);
		querySBuff.append(
			"from iccontactnotes p left join icfiledetails p2 on p.filedetailsid = p2.filedetailsid where  p.status='ACTIVE' "
		);
		...
		if (data != null && !data.trim().isEmpty()) {
			...
			querySBuff.append(
				" AND (p.filedetailsid=? OR p.name LIKE ?  OR p.worker LIKE ?  OR p.date LIKE ?  OR p.time LIKE ?  OR p.contactmethod LIKE ?  OR p.needaddress LIKE ?  OR p.summary LIKE ?  OR p.result LIKE ?  OR p.nextstep LIKE ?  OR p.caseplanprogress LIKE ? OR p.additionalinformation LIKE ?)  ORDER BY p.creationdatetime desc "
			);
