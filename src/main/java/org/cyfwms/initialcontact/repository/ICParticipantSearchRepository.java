...
public class ICParticipantSearchRepository {
	...
	public List<ICParticipantSearchResultDto> searchICParticipant(
		...
		return jdbcTemplate.query(
			...
			(rs, rowNum) ->
				new ICParticipantSearchResultDto(
	...
	private StringBuffer createSearchQuery(
		...
		querySBuff.append(
			"select p.icparticipantid ,p.filedetailsid , CONCAT(firstname,' ', surname) AS fullName, p.role,p.notes "
		);
		querySBuff.append(
			"from icparticipant p  left join participant p2 on p.participantid = p2.participantid where  p.status='ACTIVE'"
		);
		...
		if (data != null && !data.trim().isEmpty()) {
			data =
				data
					...
					.replace("[", "![");
			querySBuff.append(
				" AND (filedetailsid=? OR p.notes LIKE ? OR p.role LIKE ? )ORDER BY p.creationdatetime desc "
			);
