...
public class InitialContactController {
	...
	@GetMapping(
		value = { "/search/{clientname}/{fileNumber}/{caseworker}/{startingDate}/{status}" },
	...
	public List<ICSearchResultsDto> search(@PathVariable Map<String, String> var) {
		...
		initialContactSearchCriteriaDto.setClientName(
			("null".equals(var.get("clientname")) || var.get("clientname") == null)
				? null
				: var.get("clientname")
		);
		initialContactSearchCriteriaDto.setFileNumber(
			("null".equals(var.get("fileNumber")) || var.get("fileNumber") == null)
				? null
				: Long.parseLong(var.get("fileNumber"))
		);
	...
	public ResponseEntity<String> removeICFileDetails(
		@PathVariable("fileNumber") Long fileNumber
	) {
	...
	public ResponseEntity<ICAttachmentDTO> saveOne(
		@RequestParam(value = "file", required = false) MultipartFile file,
		@RequestParam("icDto") String icDto
	)
		throws IOException {
		ICAttachmentDTO iCAttachmentDTO = icAttachmentService.uploadAttachment(file, icDto);
		log.info(
			"Save/Upload/Put One/Single Attachment in Initial Contact :" + iCAttachmentDTO
		);
	...
	@GetMapping(
		value = "/attachments/read_all/{filedetailsid}",
		produces = "application/json"
	)
	@ResponseStatus(OK)
	public List<ICAttachmentDTO> readAll(
	...
	public void removeICAttachment(@PathVariable("icattchmentid") Long icAttachmentId) {
		log.info(
			"Soft Remove/Delete One/Single Attachment By ICAttachmentID :" + icAttachmentId
		);
