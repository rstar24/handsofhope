...
public class ICPatientCareInfoServiceImpl implements ICPatientCareInfoService {
	...
	public ICPatientCareInfoDto readAllPatientCareInfo(Long fileDetailsId) {
		...
		if (fileDetailsId != 0) {
			ICPatientCareInfo iCPatientCareInfo = iCPatientCareInfoRepository.findByFileDetailsId(
				fileDetailsId
			);
	...
	@Override
	public ICPatientCareInfoDto saveAllPatientCareInfo(
		ICPatientCareInfoDto iCPatientCareInfoDto
	) {
		...
		if (iCPatientCareInfoDto.getPatientCareInfoId() == 0) {
			...
			iCPatientCareInfo =
				iCPatientCareInfoRepository
					.findById(iCPatientCareInfoDto.getPatientCareInfoId())
					.get();
			BeanUtils.copyProperties(iCPatientCareInfoDto, iCPatientCareInfo);
		}
		...
		if (iCPatientCareInfo.getTypeOfPatient().equals("Outpatient")) {
			...
			Optional<PatientCareInfoInpatient> op = Optional.ofNullable(
				patientCareInfoInpatientRepository.findByPatientCareInfoId(
					iCPatientCareInfoDto.getPatientCareInfoId()
				)
			);
			if (op.isPresent()) {
			...
			iCPatientCareInfoDto
				.getOutpatient()
				.setOutpatientId(iCPatientCareInfo.getOutpatient().getOutpatientId());
			iCPatientCareInfoDto.setPatientCareInfoId(iCPatientCareInfo.getPatientCareInfoId());
			iCPatientCareInfoDto
				.getOutpatient()
				.setPatientCareInfoId(iCPatientCareInfo.getOutpatient().getPatientCareInfoId());
		} else {
			Optional<PatientCareInfoOutpatient> op = Optional.ofNullable(
				patientCareInfoOutpatientRepository.findByPatientCareInfoId(
					iCPatientCareInfoDto.getPatientCareInfoId()
				)
			);
			if (op.isPresent()) {
			...
			iCPatientCareInfoDto
				.getInpatient()
				.setInpatientId(iCPatientCareInfo.getInpatient().getInpatientId());
			iCPatientCareInfoDto
				.getInpatient()
				.setPatientCareInfoId(iCPatientCareInfo.getInpatient().getPatientCareInfoId());
		}
