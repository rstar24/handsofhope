...
public class ICFileDetailsServiceImpl implements ICFileDetailsService {
	...
	public ICFileDetailsDto readAllFileDetails(Long fileDetailsID) {
		...
		if (fileDetailsID != 0) {
			ICFileDetails iCFileDetails = fileDetailsRepo
				.findById(fileDetailsID)
				.orElseThrow(
					() ->
				...
				if (iCFileDetails.getStatusOfDeletion().equals("ACTIVE")) {
					...
					if (iCFileDetails.getClientName() != null) {
						Participant participant = participantRepository.findByParticipantId(
							Long.parseLong(iCFileDetailsDto.getClientName())
						);
						iCFileDetailsDto.setClientName(
	...
	public void removeICFileDetails(Long fileNumber) {
		...
		Optional<ICFileDetails> iCFileDetailsOpt = fileDetailsRepo.findByFileNumber(
			fileNumber
		);
