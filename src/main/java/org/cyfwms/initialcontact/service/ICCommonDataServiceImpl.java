...
public class ICCommonDataServiceImpl implements ICCommonDataService {
	...
	public ICCommonDataDto iCCommonData(Long fileNumber) {
		...
		if (fileNumber != 0) {
			...
			if (iCFileDetails != null) {
				...
				ICPresentConcerns iCPresentConcerns = iCPresentConcernsRepository.findByFileDetailsId(
					fileDetailsId
				);
				if (iCPresentConcerns != null) {
					iCCommonDataDto.setPresentConcerns(iCPresentConcerns);
				}
				ICReferralInfo iCReferralInfo = iCReferralInfoRepository.findByFileDetailsId(
					fileDetailsId
				);
				if (iCReferralInfo != null) {
					iCCommonDataDto.setReferralInfo(iCReferralInfo);
				}
				ICContactNotes icContactNotes = icContactNotesRepository.findByFileDetailsId(
					fileDetailsId
				);
	...
	private ICFileDetails readFileNumber(Long fileNumber) {
		...
		Optional<ICFileDetails> iCFileDetailsOpt = iCFileDetailsRepository.findByFileNumber(
			fileNumber
		);
