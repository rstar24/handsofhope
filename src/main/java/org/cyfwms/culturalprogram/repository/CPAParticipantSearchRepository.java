...
public class CPAParticipantSearchRepository {
	...
	public List<CPAParticipantSearchResultsDto> searchParticipantCulturalProgAndAct(
		CPASearchCriteriaDto CPASearchCriteriaDto
	) {
		...
		return jdbcTemplate.query(
			...
			(rs, rowNum) ->
				new CPAParticipantSearchResultsDto(
	...
	private StringBuffer createSearchQuery(
		CPASearchCriteriaDto searchCriteria,
		List<Object> argsObjectList
	) {
		StringBuffer querySBuff = new StringBuffer();
		String data = searchCriteria.getData();
		querySBuff.append(
			"select p.participantculturalprogid ,p.culturalprogramid , CONCAT(firstname,' ', surname) AS fullName, p.role,p.notes "
		);
		querySBuff.append(
			"from participantculturalprogram p  left join participant p2 on p.participantid = p2.participantid where  p.status='ACTIVE'"
		);
		if (data != null && !data.trim().isEmpty()) {
			data =
				data
					...
					.replace("[", "![");
			querySBuff.append(
				" AND (culturalprogramid=? OR p.notes LIKE ? OR p.role LIKE ? )ORDER BY p.creationdatetime desc "
			);
