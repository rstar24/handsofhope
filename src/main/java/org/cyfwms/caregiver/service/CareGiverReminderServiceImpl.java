...
public class CareGiverReminderServiceImpl implements CareGiverReminderService {
	...
	public List<CareGiverReminderDto> saveCGReminder(
		CareGiverReminderDto careGiverReminderDto
	) {
		...
		if (careGiverReminderDto.getCgReminderId() == 0) {
			...
			careGiverReminder =
				careGiverReminderRepository
					.findById(careGiverReminderDto.getCgReminderId())
					.get();
			reminder =
				reminderRepository
					.findById(careGiverReminderDto.getReminderDto().getReminderId())
					.get();
			BeanUtils.copyProperties(careGiverReminderDto.getReminderDto(), reminder);
			BeanUtils.copyProperties(careGiverReminderDto, careGiverReminder);
		}
	...
	public List<CareGiverReminderDto> checkFrequency(
		CareGiverReminderDto careGiverReminderDto
	) {
		Period pd = Period.between(
			careGiverReminderDto.getReminderDto().getReminderDate(),
			careGiverReminderDto.getReminderDto().getEndDate()
		);
		...
		if (careGiverReminderDto.getReminderDto().getFrequency().equalsIgnoreCase("Daily")) {
			...
			careGiverReminderDto.getReminderDto().getFrequency().equalsIgnoreCase("Weekly")
		) {
			n = difference / 7;
			rem = n % 7;
			n = n + 1;
			...
			careGiverReminderDto.getReminderDto().getFrequency().equalsIgnoreCase("Monthly")
		) {
			n = monthDiff + 1;
			counter = 31;
		} else {
	...
	public List<CareGiverReminderDto> saveFrequency(
		int n,
		int counter,
		int rem,
		CareGiverReminderDto careGiverReminderDto
		...
		for (int i = 0; i < n; i++) {
			...
			caregiverReminder.setStatusOfDeletion("ACTIVE");
			reminder.setStatusOfDeletion("ACTIVE");
			LocalDate localDate = careGiverReminderDto
				.getReminderDto()
				.getReminderDate()
				.plusDays(cnt);
			...
			if (
				...
				int month = careGiverReminderdto
					.getReminderDto()
					.getReminderDate()
					.getMonth()
					.getValue();
	...
	public CareGiverReminderDto readCGReminder(Long cgReminderId) {
		...
		ReminderDto reminderDto = new ReminderDto();
		if (cgReminderId != 0) {
			Optional<CareGiverReminder> careGiverReminder = careGiverReminderRepository.findById(
				cgReminderId
			);
	...
	public void removeCGReminder(Long cgReminderId) {
		...
		CareGiverReminder c = careGiverReminderRepository
			.findById(cgReminderId)
			.orElseThrow(
				() ->
