		...
		if (cpDto.getId() == 0) {
			...
			cp =
				careProviderRepository
					.findByIdWhereActive(cpDto.getId())
					.orElseThrow(
						() ->
	...
	public CareProviderDto read(Long id) {
		...
		if (id != 0) {
			...
			if (!cp.getPrimaryCaregiver().isEmpty()) {
				Participant participantPrimary = participantRepository.findByParticipantId(
					Long.parseLong(cpDto.getPrimaryCaregiver())
				);
				cpDto.setPrimaryCaregiver(
					participantPrimary.getFirstname() + " " + participantPrimary.getSurname()
				);
			...
			if (cp.getSecondaryCaregiver() != null) {
				if (!cp.getSecondaryCaregiver().isEmpty()) {
					Participant participantSecondary = participantRepository.findByParticipantId(
						Long.parseLong(cpDto.getSecondaryCaregiver())
					);
					cpDto.setSecondaryCaregiver(
						participantSecondary.getFirstname() + " " + participantSecondary.getSurname()
					);
