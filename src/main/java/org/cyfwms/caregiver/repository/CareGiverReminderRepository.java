...
@Repository
public interface CareGiverReminderRepository
	extends JpaRepository<CareGiverReminder, Long> {
	@Query(
		value = "select * from caregiverreminder where cgreminderid=? AND statusofdeletion='active'",
		nativeQuery = true
	)
	...
	@Query(
		value = "select * from caregiverreminder where  id=? AND statusofdeletion='active'",
		nativeQuery = true
	)
