...
@Repository
public interface CareGiversAppointmentRepository
	extends JpaRepository<CaregiverAppointment, Long> {
	@Query(
		value = "select * from cg_appointment where appointmentid=? AND status='active'",
		nativeQuery = true
	)
	CaregiverAppointment findByAppointmentId(long AppointmentId);

	@Query(
		value = "select * from cg_appointment where  id=? AND status='active'",
		nativeQuery = true
	)
