...
public class CareProviderSearchRepository {
	...
	private StringBuffer createSearchQuery(
		CareProviderSearchCriteriaDto searchCriteria,
		List<Object> argsObjectList
	) {
		StringBuffer querySBuff = new StringBuffer();
		querySBuff.append(
			"select c.id ,c.reference_id ,c.name ,c.type ,primary_caregiver, secondary_caregiver ,c.status "
		);
		...
		if (priCaregiver != null && !priCaregiver.trim().isEmpty()) {
			querySBuff.append(
				" left join participant p2 on c.primary_caregiver =p2.participantid "
			);
			priCaregiver =
				priCaregiver
					.trim()
		...
		if (secCaregiver != null && !secCaregiver.trim().isEmpty()) {
			querySBuff.append(
				" left join participant p2 on c.secondary_caregiver = p2.participantid"
			);
			querySBuff.append(" where c.deletion_status='ACTIVE'");
			secCaregiver =
				secCaregiver
