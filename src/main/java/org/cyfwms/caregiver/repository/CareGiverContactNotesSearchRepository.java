...
public class CareGiverContactNotesSearchRepository {
	...
	private StringBuffer createSearchQuery(
		CareGiverContactNotesSearchCriteriaDto searchCriteria,
		...
		querySBuff.append(
			"select c.cgcontactnotesid ,c.cgproviderid, c.name ,c.worker ,c.date , c.time,c.contactmethod ,c.needaddress ,c.summary ,c.result ,c.nextstep ,c.caseplanprogress ,c.additionalinformation "
		);
		querySBuff.append(
			"from caregivercontactnotes c left join cg_care_provider c2 on c.cgproviderid = c2.id where  c.status='ACTIVE' "
		);
		...
		if (data != null && !data.trim().isEmpty()) {
			...
			querySBuff.append(
				" AND (c.cgproviderid=? OR c.name LIKE ?  OR c.worker LIKE ?  OR c.date LIKE ?  OR c.time LIKE ?  OR c.contactmethod LIKE ?  OR c.needaddress LIKE ?  OR c.summary LIKE ?  OR c.result LIKE ?  OR c.nextstep LIKE ?  OR c.caseplanprogress LIKE ? OR c.additionalinformation LIKE ?)  ORDER BY c.creationdatetime desc "
			);
