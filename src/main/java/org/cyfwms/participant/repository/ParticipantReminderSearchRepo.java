...
public class ParticipantReminderSearchRepo {
	...
	private StringBuffer createSearchQuery(
		ParticipantReminderSearchCriteriaDto searchCriteriaDto,
		...
		querySBuff.append(
			"select p.reminderid ,p2.participantid ,p.assignedto ,p.regarding ,p.subject ,p.frequency ,p.status ,p.description ,p.reminderDate ,p.endDate ,p2.participantreminderid "
		);
		querySBuff.append(
			"from reminder p left join participantreminder p2 on (p.reminderid = p2.reminderid) where p2.statusofdeletion='ACTIVE' "
		);
		...
		if (data != null && !data.trim().isEmpty()) {
			...
			querySBuff.append(
				" AND (p2.participantid=? OR p.assignedto LIKE ?  OR p.regarding LIKE ?  OR p.subject LIKE ?  OR p.frequency LIKE ?  OR p.status LIKE ? OR p.description LIKE ? OR p.reminderDate LIKE ? OR p.endDate LIKE ? ) ORDER BY p2.creationdatetime desc "
			);
