...
public class ParticipantSearchRepository {
	...
	private StringBuffer createSearchQuery(
		ParticipantSearchCriteriaDto searchCriteria,
		List<Object> argsObjectList
	) {
		StringBuffer querySBuff = new StringBuffer();
		querySBuff.append(
			"select p.participantid, p.firstname, p.middlename, p.surname, p.maritalstatus,  p.dateofbirth, p2.city, p2.homephone, p2.cellphone, p2.workphone, p.referenceid "
		);
		querySBuff.append(
		...
		if (phoneNumber != null && !phoneNumber.trim().isEmpty()) {
			phoneNumber =
				phoneNumber
					...
					.replace("[", "![");
			querySBuff.append(
				" AND (p2.homephone = ? OR p2.cellphone = ? OR p2.workphone = ?)"
			);
