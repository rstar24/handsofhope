...
public class ParticipantReminderServiceImpl implements ParticipantReminderService {
	...
	public ParticipantReminderDto readParticipantReminder(Long participantReminderId) {
		...
		if (participantReminderId != 0) {
			Optional<ParticipantReminder> participantReminder = participantReminderRepository.findById(
		...
		if (participantReminderDto.getParticipantReminderId() == 0) {
			...
			participantReminder =
				participantReminderRepository
					.findById(participantReminderDto.getParticipantReminderId())
					.get();
			reminder =
				reminderRepository
					.findById(participantReminderDto.getReminderDto().getReminderId())
					.get();
			BeanUtils.copyProperties(participantReminderDto.getReminderDto(), reminder);
	...
	public List<ParticipantReminderDto> checkFrequency(
		ParticipantReminderDto participantReminderDto
	) {
		log.info("Inside CheckFrequency");
		Period pd = Period.between(
			participantReminderDto.getReminderDto().getReminderDate(),
			participantReminderDto.getReminderDto().getEndDate()
		);
		int difference = pd.getDays();
		int monthDiff = pd.getMonths();
		int n = 0, counter = 0, rem = 0;
		if (
			participantReminderDto.getReminderDto().getFrequency().equalsIgnoreCase("Daily")
		) {
			n = difference + 1;
			rem = n + 1;
			counter = 1;
		} else if (
			participantReminderDto.getReminderDto().getFrequency().equalsIgnoreCase("Weekly")
		) {
			n = difference / 7;
			rem = n % 7;
			n = n + 1;
			...
			participantReminderDto.getReminderDto().getFrequency().equalsIgnoreCase("Monthly")
		) {
			n = monthDiff + 1;
			counter = 31;
		} else {
	...
	public List<ParticipantReminderDto> saveFrequency(
		int n,
		int counter,
		int rem,
		ParticipantReminderDto participantReminderDto
		...
		for (int i = 0; i < n; i++) {
			...
			participantReminder.setStatusOfDeletion("ACTIVE");
			reminder.setStatusOfDeletion("ACTIVE");
			LocalDate localDate = participantReminderDto
				.getReminderDto()
				.getReminderDate()
				.plusDays(cnt);
			...
			if (
				...
				int month = participantReminderdto
					.getReminderDto()
					.getReminderDate()
					.getMonth()
					.getValue();
				int monthly = frequencyGeneratorUtil.generateMonthlyUtil(
					month,
					cnt,
					year,
					counter
	...
	public void removeParticipantReminder(Long participantReminderId) {
		...
		ParticipantReminder p = participantReminderRepository
			.findById(participantReminderId)
			.orElseThrow(
				() ->
