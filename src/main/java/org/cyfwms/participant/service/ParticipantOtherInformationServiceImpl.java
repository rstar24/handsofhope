...
@AllArgsConstructor
@Slf4j
public class ParticipantOtherInformationServiceImpl
	implements ParticipantOtherInformationService {
	@Autowired
	...
	public ParticipantOtherInformationServiceDto readParticipantOtherInformation(
		...
		if (participantId != 0) {
			ParticipantOtherInformation participantOtherInformation = participantOtherInformationRepository.findByParticipantId(
				participantId
			);
	...
	public ParticipantOtherInformationServiceDto saveParticipantOtherInformation(
		...
		if (participantOtherInformationServiceDto.getParticipantOtherInfoId() == 0) {
			...
			participantOtherInformation =
				participantOtherInformationRepository
					.findById(participantOtherInformationServiceDto.getParticipantOtherInfoId())
					.get();
			BeanUtils.copyProperties(
				participantOtherInformationServiceDto,
				participantOtherInformation
		...
		participantOtherInformation =
			participantOtherInformationRepository.save(participantOtherInformation);
		participantOtherInformationServiceDto.setParticipantOtherInfoId(
			participantOtherInformation.getParticipantOtherInfoId()
		);
