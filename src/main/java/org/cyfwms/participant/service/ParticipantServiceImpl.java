...
public class ParticipantServiceImpl implements ParticipantService {
	...
	public ParticipantIdentityDto readParticipantIdentity(Long participantId) {
		...
		if (participantId != 0) {
			...
			if (participant.getStatus().equals("ACTIVE")) {
				...
				Optional<ParticipantAttachment> participantAttachmentOpt = participant
					.getParticipantAttachmentList()
					.stream()
					.filter(
						attachment ->
				...
				if (participantAttachmentOpt.isPresent()) {
					BeanUtils.copyProperties(
						participantAttachmentOpt.get(),
						participantAttachmentDto
					);
				}
				BeanUtils.copyProperties(participant, participantIdentityDto);

				mapParticipantImageData(participantIdentityDto, participantAttachmentDto);
			}
		}
	...
	@Override
	public ParticipantIdentityDto saveParticipantIdentity(
		ParticipantIdentityDto participantIdentityDto,
		...
		if (multipartFile != null) {
			...
			participantAttachmentDto = populateParticipantAttachmentData(multipartFile);
			...
			if (participantAttachmentList != null) {
				for (int i = 0; i < participant.getParticipantAttachmentList().size(); i++) {
					participantAttachmentDto.setParticipantAttachmentId(
						participant.getParticipantAttachmentList().get(i).getParticipantAttachmentId()
					);
					participantAttachmentDto
						.getAttachment()
						.setAttachmentId(
	...
	private ParticipantAttachmentDto removeProfilePicture(Participant participant) {
		...
		for (int i = 0; i < participant.getParticipantAttachmentList().size(); i++) {
			...
			participantAttachmentDto.setName(
				participant.getParticipantAttachmentList().get(i).getName()
			);
			participantAttachmentDto.setType(
				participant.getParticipantAttachmentList().get(i).getType()
			);
			participantAttachmentDto.setAttachmentType(
				participant.getParticipantAttachmentList().get(i).getAttachmentType()
			);
		}
	...
	private void validateParticipantImage(MultipartFile multipartFile) {
		...
		if (invalidParticipantImage) {
			throw new ResponseStatusException(
				...
				"JPG PNG and JPEG content type are allowed"
			);
		}
	}
