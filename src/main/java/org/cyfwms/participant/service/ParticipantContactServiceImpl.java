...
public class ParticipantContactServiceImpl implements ParticipantContactService {
	...
	public ParticipantContactDto readParticipantContact(Long participantId) {
		...
		if (participantId != 0) {
			ParticipantContact participantContact = participantContactRepository.findByParticipantId(
				participantId
			);
	...
	@Override
	public ParticipantContactDto saveParticipantContact(
		...
		if (participantContactDto.getParticipantContactId() == 0) {
			...
			participantContact =
				participantContactRepository
					.findById(participantContactDto.getParticipantContactId())
					.get();
			BeanUtils.copyProperties(participantContactDto, participantContact);
		}
		participantContact = participantContactRepository.save(participantContact);
		participantContactDto.setParticipantContactId(
			participantContact.getParticipantContactId()
		);
