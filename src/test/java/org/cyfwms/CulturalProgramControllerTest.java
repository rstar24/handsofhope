...
public class CulturalProgramControllerTest {
	...
	public void readCpaIdentityTest() throws Exception {
		CPAIdentityDto cpaIdentityDto = CPAIdentityDto
			.builder()
			...
			.endDate(LocalDate.of(2022, 06, 04))
			.totalParticipation("two")
			.type("culture")
			.build();
		...
		mockMvc
			.perform(get("/v1/culturalprogandactservice/readCulturalProgAndAct/1"))
			.andDo(print())
			...
			.andExpect(
				MockMvcResultMatchers.jsonPath("$.costOrParticipationDetails").value("nodetail")
			)
			...
			.andExpect(
				MockMvcResultMatchers
					.jsonPath("$.startDate")
					.value(LocalDate.of(2022, 04, 01).toString())
			)
			.andExpect(
				MockMvcResultMatchers
					.jsonPath("$.endDate")
					.value(LocalDate.of(2022, 06, 04).toString())
			)
	...
	public void saveCpaIdentityTest() throws Exception {
		...
		mockMvc
			.perform(
				put("/v1/culturalprogandactservice/saveCulturalProgAndAct")
	...
	public void searchCulturalProgAndActTest() throws Exception {
		...
		cpaSearchResultsDtoList.add(
			CPASearchResultsDto
				.builder()
				.CulturalProgramId(2L)
				.name("ayush")
				.type("worker")
				.caseworker("yes")
				.referenceId(3L)
		...
		Mockito
			.when(CPASearchService.searchCulturalProgAndAct(any()))
			.thenReturn(cpaSearchResultsDtoList);
		mockMvc
			.perform(
				get(
						"/v1/culturalprogandactservice/culturalProgAndActSearch/null/ayush/null/null/null/null"
					)
			...
			.andExpect(status().isOk())
			.andDo(print())
			.andExpect(jsonPath("$.size()", is(cpaSearchResultsDtoList.size())));
	}
	...
	public void removeCpaIdentityTest() throws Exception {
		...
		mockMvc
			.perform(delete("/v1/culturalprogandactservice/removeCulturalProgAndAct/1"))
			.andExpect(status().isOk())
			.andDo(print());
	}
	...
	public void readCpaParticipantTest() throws Exception {
		CPAParticipantDto cpaParticipantDto = CPAParticipantDto
			.builder()
			.participantId(1L)
			.culturalProgramId(2L)
			.participantCulturalProId(3L)
		...
		Mockito
			.when(CPAParticipantService.readCpaParticipant(any()))
			.thenReturn(cpaParticipantDto);
		mockMvc
			.perform(get("/v1/culturalprogandactservice/readParticipantsCulturalAndAct/2"))
			.andDo(print())
			.andExpect(MockMvcResultMatchers.jsonPath("$.culturalProgramId").exists())
			...
			.andExpect(
				MockMvcResultMatchers.jsonPath("$.participant").value("participantcontact")
			)
	...
	public void saveCpaParticipantTest() throws Exception {
		...
		Mockito
			.when(CPAParticipantService.saveCpaParticipant(any()))
			.thenReturn(cpaParticipantDto);
		mockMvc
			.perform(
				put("/v1/culturalprogandactservice/saveParticipantCulturalProg")
	...
	public void removeCpaParticipantTest() throws Exception {
		...
		willDoNothing()
			.given(CPAParticipantService)
			.removeCpaParticipant(participantCulturalProId);

		mockMvc
			.perform(delete("/v1/culturalprogandactservice/removeParticipantCulturalProg/3"))
			.andExpect(status().isOk())
			.andDo(print());
	}
	...
	public void searchCulturalProgramTest() throws Exception {
		...
		cpaParticipantSearchResultsDtoList.add(
			CPAParticipantSearchResultsDto
				.builder()
				.culturalProgramId(1L)
				.participant("participant")
				.role("admin")
				.participantCulturalProId(4L)
				.notes("successfull")
				.build()
		);

		Mockito
			.when(CPAParticipantSearchService.searchParticipantCulturalProgAndAct(any()))
			.thenReturn(cpaParticipantSearchResultsDtoList);

		mockMvc
			.perform(
				get("/v1/culturalprogandactservice/participantCulturalProgSearch/4")
					.content(objectMapper.writeValueAsString(cpaParticipantSearchResultsDtoList))
					.contentType(MediaType.APPLICATION_JSON)
			)
			.andExpect(status().isOk())
			.andDo(print())
			.andExpect(jsonPath("$.size()", is(cpaParticipantSearchResultsDtoList.size())));
	}
}
