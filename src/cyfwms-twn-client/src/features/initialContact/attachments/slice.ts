...
export const doGetOne = createAsyncThunk<Record, number>(
  ...
  async (icFileID, { getState }) => {
    const store = getState() as RootState;
    const res: AxiosResponse = await doGetOneAPI(icFileID, store.login.token);
    // Becomes the `fulfilled` action payload:
    return res.data;
  }
);
...
export const doDelete = createAsyncThunk<any, number>(
  ...
  async (icFileId, { getState }) => {
    const store = getState() as RootState;
    const res: AxiosResponse = await doDeleteAPI(icFileId, store.login.token);
    // Becomes the `fulfilled` action payload:
    return res.data;
  }
);
